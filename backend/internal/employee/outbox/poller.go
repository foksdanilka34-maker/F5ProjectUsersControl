...
func NewPoller(txManager core.TxManager, publisher EventPublisher, cfg PollerConfig) *Poller {
...
func (p *Poller) Start(ctx context.Context) {
	log.Printf("[Outbox Poller] Starting poller with %d workers, batch size %d, interval %v",
		p.cfg.WorkerCount, p.cfg.BatchSize, p.cfg.PollInterval)
	...
	for {
		select {
		case <-ctx.Done():
			log.Println("[Outbox Poller] Context cancelled, shutting down poller...")
			close(tasksChan)
			wg.Wait()
			return
		case <-ticker.C:
...
func (p *Poller) pollBatch(ctx context.Context, tasksChan chan<- dto.OutboxRecord) {
	...
	err := p.txManager.WithinTx(ctx, func(r core.Repositories) error {
		var err error
	...
	if err != nil {
		if ctx.Err() == nil {
			log.Printf("[Outbox Poller] Error fetching pending batch: %v", err)
		}
		return
	}
...
func (p *Poller) processRecord(ctx context.Context, record dto.OutboxRecord) {
	...
	_ = p.txManager.WithinTx(ctx, func(r core.Repositories) error {
		if err != nil {
			log.Printf("[Outbox Poller] Failed to publish event %s (%s): %v", record.ID, record.EventType, err)
