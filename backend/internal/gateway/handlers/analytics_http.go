...
func (h *AnalyticsHTTPHandler) RegisterRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("GET /api/v1/analytics/employees", h.ListAllEmployeeMetrics)
	mux.HandleFunc("GET /api/v1/analytics/projects", h.ListAllProjectMetrics)
	mux.HandleFunc("GET /api/v1/analytics/trends", h.GetTrends)
}
...
func (h *AnalyticsHTTPHandler) ListAllEmployeeMetrics(w http.ResponseWriter, r *http.Request) {
	pageSize := int32(50)
	pageNumber := int32(1)
	resp, err := h.client.ListEmployeeMetrics(r.Context(), &pb.ListEmployeeMetricsRequest{
		PageSize:   &pageSize,
		PageNumber: &pageNumber,
	})
	if err != nil {
		if r.Context().Err() != context.Canceled && status.Code(err) != codes.Canceled {
			log.Println("list employee metrics error:", err)
		}
		http.Error(w, `{"error":"`+err.Error()+`"}`, http.StatusInternalServerError)
		return
	}

	metrics := make([]map[string]interface{}, len(resp.Metrics))
...
func (h *AnalyticsHTTPHandler) ListAllProjectMetrics(w http.ResponseWriter, r *http.Request) {
	pageSize := int32(50)
	pageNumber := int32(1)
	resp, err := h.client.ListProjectMetrics(r.Context(), &pb.ListProjectMetricsRequest{
		PageSize:   &pageSize,
		PageNumber: &pageNumber,
	})
	if err != nil {
		if r.Context().Err() != context.Canceled && status.Code(err) != codes.Canceled {
			log.Println("list project metrics error:", err)
		}
		http.Error(w, `{"error":"`+err.Error()+`"}`, http.StatusInternalServerError)
		return
	}
...
func (h *AnalyticsHTTPHandler) GetTrends(w http.ResponseWriter, r *http.Request) {
	resp, err := h.client.GetProductivityTrends(r.Context(), &pb.GetProductivityTrendsRequest{
		Period: pb.Period_PERIOD_DAILY,
		Limit:  30,
	})
	if err != nil {
		if r.Context().Err() != context.Canceled && status.Code(err) != codes.Canceled {
			log.Println("trends error:", err)
		}
		http.Error(w, `{"error":"`+err.Error()+`"}`, http.StatusInternalServerError)
		return
	}
	...
	for i, e := range resp.Entries {
		...
		entries[i] = map[string]interface{}{
			...
			"avg_completion_rate": e.AvgCompletionRate,
		}
	}

	writeJSON(w, http.StatusOK, map[string]interface{}{
		"entries": entries,
		"period":  resp.Period.String(),
	})
}
