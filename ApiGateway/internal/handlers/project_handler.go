...
func (h *ProjectHandler) GetTaskStatusHistory(c *gin.Context) {
	...
	var req models.GetTaskStatusHistoryRequest
	if err := c.ShouldBindQuery(&req); err != nil {
		log.Printf("GetTaskStatusHistory validation error: %v", err)
		response.BadRequest(c, "Invalid request: "+err.Error())
		return
	}
	...
	log.Printf("Getting task status history: taskID=%s", taskID)

	protoReq := &projectpb.GetTaskStatusHistoryRequest{
		TaskId:     taskID,
		PageSize:   req.PageSize,
		PageNumber: req.PageNumber,
	}

	history, err := h.projectService.GetTaskStatusHistory(c.Request.Context(), protoReq)
	if err != nil {
		log.Printf("GetTaskStatusHistory service error: %v", err)
		response.InternalServerError(c, "Failed to get task status history: "+err.Error())
		return
	}

	response.Success(c, http.StatusOK, history, "Task status history retrieved successfully")
}
...
func (h *ProjectHandler) GetProjectMetrics(c *gin.Context) {
	...
	var req models.GetProjectMetricsRequest
	if err := c.ShouldBindQuery(&req); err != nil {
		log.Printf("GetProjectMetrics validation error: %v", err)
		response.BadRequest(c, "Invalid request: "+err.Error())
		return
	}

	log.Printf("Getting project metrics: projectID=%s", projectID)

	protoReq := &projectpb.GetProjectMetricsRequest{
		ProjectId: projectID,
	}

	if req.StartDate != nil {
		protoReq.StartDate = timestamppb.New(*req.StartDate)
	}
	if req.EndDate != nil {
		protoReq.EndDate = timestamppb.New(*req.EndDate)
	}

	metrics, err := h.projectService.GetProjectMetrics(c.Request.Context(), protoReq)
	if err != nil {
		log.Printf("GetProjectMetrics service error: %v", err)
		response.InternalServerError(c, "Failed to get project metrics: "+err.Error())
		return
	}
